...
export default function InputScreen({
  ...
  return (
    <motion.div
      className="w-full max-w-lg px-4 py-12"
      variants={pv}
      initial="initial"
      animate="animate"
      exit="exit"
    >
      {/* ヘッダー */}
      <h1 className="text-3xl font-bold text-neutral-900 tracking-tight hidden">
        Furui
      </h1>

      <div className="mb-6">
        <p className="text-neutral-900 text-xl font-semibold text-center">
      ...
      <div className="relative mb-6 flex">
        <textarea
          ...
          disabled={isAtMax}
          className="w-full rounded-xl border border-neutral-200 bg-white px-4 pt-3 pb-12 pr-4 text-sm text-neutral-900 placeholder-neutral-400 outline-none focus:ring-2 focus:ring-neutral-800 focus:border-transparent transition disabled:opacity-40 disabled:cursor-not-allowed resize-none overflow-hidden leading-relaxed"
          aria-label="項目を入力"
        />
        <motion.button
          onClick={handleAdd}
      ...
      <div className="mt-4">
        ...
        <motion.button
          ...
          whileTap={reducedMotion ? {} : { scale: 0.97 }}
          className="flex items-center justify-center gap-3 w-full py-4 rounded-full bg-neutral-800 text-white text-sm hover:bg-neutral-600 transition disabled:opacity-30 disabled:cursor-not-allowed"
        >
          フルイにかける
        </motion.button>
      </div>
